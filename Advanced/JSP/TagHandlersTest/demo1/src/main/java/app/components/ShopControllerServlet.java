...
public class ShopControllerServlet extends HttpServlet{
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(model.authenticate(custId, pwd)){
            request.setAttribute("login", model);
            request.getRequestDispatcher("/details-view.jsp").forward(request, response);
        }else{
            request.setAttribute("problem", "Authentication Failed!");
            request.getRequestDispatcher("/index-view.jsp").forward(request, response);
        }
    }
}
